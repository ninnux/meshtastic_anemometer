...
// ============================================================
// COMPASS – Setup e calibrazione
// ============================================================
void setupCompass() {
  imu.begin();
  compass.init();

  // Controlla se esiste una calibrazione salvata
  ...
  if (calibrated) {
    prefs.begin("compass", true);
    int xMin = prefs.getInt("xMin", 0);
    int xMax = prefs.getInt("xMax", 0);
    int yMin = prefs.getInt("yMin", 0);
    int yMax = prefs.getInt("yMax", 0);
    prefs.end();
    compass.setCalibration(xMin, xMax, yMin, yMax);
    Serial.println("Calibrazione bussola caricata dall'EEPROM.");
  } else {
